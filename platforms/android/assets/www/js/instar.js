...
$(".mood_preview").addClass("bubble-pop");$(this).removeClass("bubble-pop");
...
$("#mood2").on("click","#btnCreate",function(){
console.log("click");
$("#textSelection").html($("#moodTextInput").html());
$(".quotes").addClass("bubble-pop");
new_collection();});
...
$("#quotesWrapper").off();
$("#quotesWrapper").on("click",".quotes",function(){$("#textSelection").html($(this).html());
//document.querySelector('#myNavigator').once("ons-postpop", function(){createCollection(3)});

$(".quotes").addClass("bubble-pop");
$(this).removeClass("bubble-pop");
...
set_user_data(1,"text_content",get_text(true,"textSelection"));
...
console.log("fromHistory"+image_nr);
...
var $items = $("<div class='quotes'>"+element+"</div>").css({"margin-left":3+(Math.floor(Math.random() * 11))+"vw"});
...
if($collectionGrid){$collectionGrid.masonry('destroy');}
$("#collection_area").html("");
if(!history){ak_image_nr_collection=image_chain.length;}
else if(history == 1){ak_image_nr_collection=max(0,ak_image_nr_collection-(collection_size*collection_size));}
else if(history == 2){ak_image_nr_collection = ak_image_nr_collection+(collection_size*collection_size);}
...
$("#detect-area").on('swipeup', function(){collectionHandler(NaN,true);});
$("#detect-area").off('swipedown');
$("#detect-area").on('swipedown', function(){collectionHandler(NaN,true);});
